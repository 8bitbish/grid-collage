...
console.log('\n== being quick has not cost offline ==');
{
  ...
  // Body carries the colour of the strip outside the web view on an installed
  // iPhone, which is black since the redesign, like the bar beneath it. An
  // unstyled body is transparent — rgba(0, 0, 0, 0) — so an opaque black still
  // says the stylesheet arrived. The check is unchanged: that the cached
  // stylesheet is still being applied with no network.
  ok('with its stylesheet', dark.styled==='rgb(0, 0, 0)', dark.styled);
