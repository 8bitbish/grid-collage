  ...
  document.querySelectorAll('#pm-grid > *').forEach((el)=>{
    if (el.classList.contains('pm-day')) {
      out.push({ when: el.querySelector('.pm-event-when')?.textContent ?? el.textContent,
                 ...
                 n: el.dataset.event ?? '', names: [] });
    } else if (out.length) {
      out[out.length-1].photos = el.querySelectorAll('.pm-item').length;
      out[out.length-1].names = [...el.querySelectorAll('.pm-pick')]
        .map((t)=>(t.getAttribute('aria-label').match(/e\d-(?:burst-)?\d+/) || ['?'])[0]);
    }
  });
  return out;});
